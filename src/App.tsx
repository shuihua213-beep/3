...
import { Emotion, initEmotionsFromJSON, getAllEmotions, addEmotion as addEmotionToDB } from './idb';

export type { AddEmotionParams } from './idb';
...
const FALLBACK_EMOTION: Emotion = {
  id: '__unknown__',
  name: '未知',
  emoji: '❓',
  color: '#A8A8A8',
  ...
  particle: '❓',
  questions: ['请描述你的感受？', '这种感觉持续多久了？', '有什么可以改善的吗？', '你想对现在的自己说什么？', '你希望明天有什么不同？'],
  greeting: '记录你的心情吧。'
};
...
export default function App() {
  ...
  const refreshEmotions = useCallback(async () => {
    const loaded = await getAllEmotions();
    setEmotions(loaded);
    return loaded;
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        const loaded = await initEmotionsFromJSON();
        setEmotions(loaded);
      } catch (err) {
        console.error('Failed to initialize emotions from IndexedDB:', err);
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  useEffect(() => {
    (window as any).addEmotion = async (params: any) => {
      try {
        await addEmotionToDB(params);
        await refreshEmotions();
        console.log(`Emotion "${params.id}" added successfully!`);
      } catch (err: any) {
        console.error('addEmotion failed:', err.message);
      }
    };
    return () => {
      delete (window as any).addEmotion;
    };
  }, [refreshEmotions]);
  ...
  const viewDiaryDetail = (diary: DiaryEntry) => {
    const emotion = emotions.find(e => e.id === diary.emotionId) || FALLBACK_EMOTION;
    setSelectedEmotion(emotion);
    ...
    return diaries.filter(diary => {
      ...
      if (filters.search) {
        ...
        const matchesSearch = diary.content.toLowerCase().includes(searchLower) ||
                              ...
                              (emotion?.name || diary.emotionId).includes(filters.search);
  ...
  }, [diaries, filters, emotions]);

  const stats = useMemo(() => {
    const emotionCounts: Record<string, number> = {};
    diaries.forEach(d => {
      const emotionId = emotions.find(e => e.id === d.emotionId) ? d.emotionId : FALLBACK_EMOTION.id;
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white/10 rounded-xl p-4 text-center">
              <div className="text-3xl">{(emotions.find(e => e.id === stats.mostCommon) || FALLBACK_EMOTION).emoji}</div>
              <div className="text-white font-semibold">{(emotions.find(e => e.id === stats.mostCommon) || FALLBACK_EMOTION).name}</div>
          ...
          <div className="space-y-3">
            {[...emotions, ...(stats.emotionCounts[FALLBACK_EMOTION.id] ? [FALLBACK_EMOTION] : [])].map(emotion => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-sky-400 via-cyan-500 to-teal-500 flex items-center justify-center">
        <div className="text-center">
